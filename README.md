...
5. Настрой HTTPS (обязательно — и для Telegram webhook, и для вебхуков Click/Payme),
   ...
   Прокси должен передавать заголовки `X-Forwarded-Proto`/`X-Forwarded-For`, а в `.env`
   должен быть выставлен `TRUST_PROXY` (по умолчанию `1` — один прокси). Иначе Express
   считает соединение незащищённым, `secure`-кука сессии админки вообще не отправляется
   браузеру, и вход в панель падает с ошибкой «Invalid CSRF token»: форма логина и её
   проверка оказываются в разных сессиях. Если Node смотрит в интернет напрямую, без
   прокси — поставь `TRUST_PROXY=false`.
...
- [ ] В кабинете Payme Business проверены: URL эндпоинта `https://<WEB_BASE_URL>/payments/payme`,
      имя поля `account` (код должен называться ровно `merchant_trans_id`), и то, каким ключом
      касса сейчас подписывает запросы (до активации — тестовым), а также включены ли
      фискальные чеки (тогда нужен `detail` в ответе, см. ниже)
...
- **Payme временно отключён** (`PAYME_ENABLED=false`) — кнопка скрыта из бота, но вебхук
  ...
  - **Авторизация**: Payme подписывает запросы Basic-заголовком `Paycom:<ключ кассы>`.
  Сверяется только пароль, и только с непустыми ключами: если `PAYME_TEST_KEY` не заполнен,
  он больше не «совпадает» с пустым паролем (раньше это позволяло кому угодно дёргать
  `PerformTransaction`). Ключ может содержать двоеточие — заголовок режется по первому.
  **До активации кассы Payme подписывает запросы ТЕСТОВЫМ ключом** (`PAYME_TEST_KEY`),
  после активации — боевым (`PAYME_SECRET_KEY`); держи заполненным тот, которым касса
  реально подписывает, иначе все запросы получают `-32504`.
  - **Диагностика**: `/payments/payme` пишет в `logs/webhooks.log` (см.
  `src/utils/webhookLogger.js`) метод, результат проверки авторизации, список пришедших
  заголовков и итоговый JSON-RPC-ответ; секреты туда не попадают — только схема, логин
  (`Paycom`) и длина ключа. Стартовый лог показывает `secret_key_set`/`test_key_set` и длины
  обоих ключей. Ответ всегда HTTP 200 с валидным JSON-RPC, даже при исключении внутри
  хендлера — любой другой код/невалидное тело Payme трактует как `-32400` и показывает юзеру
  «Сервис поставщика услуг работает некорректно» (тот же текст, что и при `-32504`).
  - **Фискализация не реализована**: `CheckPerformTransaction` возвращает только
  `{ allow: true }`. Если в кабинете Payme у кассы включены фискальные чеки, туда
  понадобится добавить `detail` (`receipt_type` + `items` с `title`, `price`, `count`,
  `code` (ИКПУ), `package_code`, `vat_percent`) — значения берутся из карточки услуги в
  налоговой (soliq.uz), придумывать их нельзя.
- **Click оплачивается по ссылке** (`src/payments/click.linkBuilder.js` строит
  ...
  Payme — минимальными единицами (тийины, ×100). Пересчёт учтён в `payme.js` (`tiyinToUzs`).
...
- **Оплата "как за коммуналку" работает с холодного старта** — юзер может открыть
  ...
  ищут по `merchant_trans_id`/`account` **живой (`pending`) платёж именно своего провайдера**,
  а если не находят — пробуют найти юзера по этому же значению как по коду и заводят платёж
  на лету по текущей цене канала. Поиск обязательно провайдерный: код юзера один и тот же для
  всех провайдеров, и до фикса строка, заведённая Click, навсегда занимала этот код — Payme
  по нему отвечал `-31050`, а Click `-5`. Поэтому с миграции `...0017` на
  `payments.merchant_trans_id` больше нет глобального `UNIQUE`: вместо него частичный
  уникальный индекс `(provider, merchant_trans_id) WHERE status = 'pending'` — живой заказ по
  коду у провайдера ровно один, а история оплаченных/отменённых попыток копится рядом. Единая точка идентификации клиента везде одна — `users.code`, независимо от того,
