...
async function guardUserState(ctx, user) {
  // Мягкое удаление теперь реально отключает юзера (а не только прячет из списка в
  // админке) — проверяем раньше бана: удалённый аккаунт логически "более выключен",
  // чем просто заблокированный (админ мог сначала удалить, не снимая старый блок).
  if (user.deleted_at) {
    await ctx.reply(t(user.language, 'account_deleted'), html());
    return false;
  }

  // Бан — независимый флаг, а не значение status: проверяем его первым (после удаления),
  // чтобы заблокированный paid-юзер видел сообщение о блокировке, а не "уже оплачено".
  if (user.blocked_at) {
